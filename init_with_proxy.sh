...
install_oh_my_zsh() {
    ...
    if [ -d "$HOME/.oh-my-zsh" ]; then
        ...
        sh -c "$(proxy curl -fsSL https://raw.githubusercontent.com/robbyrussell/oh-my-zsh/master/tools/install.sh)" "" --unattended
...
install_zsh_plugins() {
    ...
    if [ ! -d "${ZSH_CUSTOM_PLUGINS}/zsh-autosuggestions" ]; then
        proxy git clone https://github.com/zsh-users/zsh-autosuggestions "${ZSH_CUSTOM_PLUGINS}/zsh-autosuggestions"
    else
    ...
    if [ ! -d "${ZSH_CUSTOM_PLUGINS}/zsh-syntax-highlighting" ]; then
        proxy git clone https://github.com/zsh-users/zsh-syntax-highlighting.git "${ZSH_CUSTOM_PLUGINS}/zsh-syntax-highlighting"
    else
...
install_autojump() {
    ...
    if command -v "autojump" &> /dev/null; then
        ...
        local temp_dir
        temp_dir=$(mktemp -d)
        proxy git clone https://github.com/wting/autojump.git "$temp_dir"
        cd "$temp_dir"
        python3 ./install.py
        cd "$HOME"
        rm -rf "$temp_dir"
...
install_fzf() {
    ...
    if [ -d "$HOME/.fzf" ]; then
        ...
        proxy git clone --depth 1 https://github.com/junegunn/fzf.git ~/.fzf
        ~/.fzf/install --all
...
install_miniforge() {
    ...
    if [ -d "$install_path" ]; then
        ...
        proxy curl -L -o "$INSTALLER_SCRIPT" "$MINIFORGE_URL"
